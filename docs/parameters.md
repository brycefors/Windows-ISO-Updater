...
| `-WindowsVersion` | Windows version to download when no ISO is supplied: `10` or `11`. Defaults to `11`. Once an ISO is mounted, catalog searches use the version the image itself reports, not this parameter, so it does not need to match media supplied with `-IsoPath`. |
| `-Server` | Service **Windows Server** media (2016 through 2025) instead of a client ISO. Only needed to skip the automatic download attempt, since neither Fido nor the Media Creation Tool serves Server media, so without it you must supply Server media yourself with `-IsoPath` or drop it into the download folder. Once an ISO is mounted, Server versus client is detected from the image itself, so `-IsoPath` pointed at Server media works without this switch too, and the catalog is searched for the right family either way. `-WindowsVersion`, `-Release` and `-Language` are ignored once Server media is detected. A mismatch between this switch and what the media turns out to be is only a warning, the run continues with what was actually found. |
...
| `-KeepEditions` | Editions to **keep** in the final ISO, removing the rest to slim it down. Accepts edition names (partial matches allowed) or index numbers, comma-separated. Overrides the default of keeping Enterprise, Pro and Home. If none of the values match any edition, all editions are kept instead (equivalent to `-KeepAllEditions`). A partial mismatch (some values match, some don't) exits with error 1. |
...
| `-BaselineOnly` | Skips the cumulative update in hotpatch non-baseline months (February, March, May, June, August, September, November, December). In baseline months (January, April, July, October) the cumulative update is integrated normally. Use only for Windows 11 Enterprise 25H2 media enrolled in Intune or Azure Arc hotpatch, and does not apply to Windows 11 LTSC media, which has no hotpatch support. On Home or Pro media this would leave the ISO underpatched in non-baseline months. |
...
| `-OutputIsoPath` | Where to write the recompiled ISO. Give it a **full file path** to name the ISO yourself, or a **folder** to keep the generated name and only change where it lands. Defaults to the `Output\` folder under the working folder, named after the contents, the image's locale, and the patched build, e.g. `Win11_Pro_x64_enGB_26100.4061_20260815-1332.iso`. The destination is created and test-written before the build starts, so somewhere that cannot be written stops the run in seconds rather than after hours. A network destination is built locally first and copied across when it is finished. |
| `-IsoNamePrefix` | Text to prepend to the auto-generated ISO file name. For example, `"Company-"` produces `Company-Win11_Pro_x64_enGB_....iso`. Has no effect when `-OutputIsoPath` is a full file path. |
| `-IsoNameSuffix` | Text to append to the auto-generated ISO file name base, before the `.iso` extension. For example, `"-Unattended"` produces `Win11_Pro_x64_enGB_...-Unattended.iso`. Has no effect when `-OutputIsoPath` is a full file path. |
| `-VolumeLabel` | Volume label written into the finished ISO, which is what File Explorer shows and what Rufus and Ventoy copy onto the USB stick. Defaults to a label describing the contents, e.g. `WIN11_ENTPRO_X64_ENGB_26100_4652`. Maximum 32 characters, letters, digits, `_`, `-` and `.` only, no spaces. |
