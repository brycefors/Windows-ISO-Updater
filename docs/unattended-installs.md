...
> [!CAUTION]
> On hardware that is **not** Autopilot-enrolled, removing the automatic sign-in means a person has to complete OOBE's account-creation screens by hand before FirstLogonCommands (driver dispatch, VMware Tools install, `Windows.old` cleanup) can run. This file is no longer zero-touch past the disk wipe and image apply on non-Autopilot hardware. Remote Desktop is still reachable on all firewall profiles here, same as `autounattend-lab-admin.xml`.

Six things differ from the lab example:
