...
The two files in [`Examples/`](../Examples) are a matched pair: one deploys a ready-to-use machine, the other builds an image to deploy *from*. Both were produced with [schneegans.de/windows/unattend-generator](https://schneegans.de/windows/unattend-generator/) and then **hand edited**; a comment at the top of each records exactly what was changed and why. The generator URL preserved alongside them reproduces the original options only — regenerating from it discards the manual changes.
