...
* `last-build.json` — the newest build. This is the one the next run compares against.
* `History\stamp_<date>_<time>.json` — one copy per build, newest `-StampHistoryCount` kept (default 30).
