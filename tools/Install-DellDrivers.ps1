...
.PARAMETER DcuVariant
    ...
    Universal for the Windows Universal Application. Defaults to Win32. The silent-install switch is
    confirmed identical for both variants; dcu-cli.exe discovery and CLI behavior after installation
    have only been verified against Win32, and remain unverified for the Universal variant.
...
.EXAMPLE
    ...
    installing or running its CLI. The silent-install switch is confirmed identical to Win32's, but
    dcu-cli.exe discovery for this variant is unverified by this script beyond the download step.
...
param(
    ...
    [Parameter(HelpMessage = "Which Dell Command Update catalog entry to resolve: Win32 for the classic desktop installer, or Universal for the Windows Universal Application. Defaults to Win32; the silent-install switch is confirmed identical for both variants, but dcu-cli.exe discovery has only been verified against Win32.")]
...
if ($DcuCliPath) {
    ...
    elseif ($InstallProcess.ExitCode -in @(2, 3010)) {
        Write-Host '  Installed successfully (reboot required)' -ForegroundColor Yellow
        $script:RebootRequired = $true
    }
    else {
