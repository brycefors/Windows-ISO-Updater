...
| `-MctEdition` | Edition passed to MCT's `/MediaEdition` switch (e.g. `Professional`, `Enterprise`, `Education`). Unset by default: that switch makes MCT demand a product key, and without it MCT builds the usual multi-edition ISO that `-Edition`/`-KeepEditions` can pick from. |
